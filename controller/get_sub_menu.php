<?php
    ...
    $rows = $get_item->fetch_details_cond('sub_menus', 'sub_menu_id', $id);
     if(gettype($rows) == 'array'){
        ...
        //get current package name of item
        $pks = $get_menu->fetch_details_group('packages', 'package', 'package_id', $row->package);
        $package = $pks->package;
    ?>
    <div class="add_user_form priceForm" style="width:100%">
        ...
        <section style="text-align:left">
            <div class="inputs" style="justify-content:left">
                ...
                <div class="data" style="width:30%">
                    <label for="menu" style="text-align:left!important;">Select package</label>
                    <select name="package" id="package" required>
                        <option value="<?php echo $row->package?>"selected required><?php echo $package?></option>
                        <?php
                            
                            $pkss = $get_item->fetch_details_order('packages', 'package');
                            foreach($pkss as $pk){
                        ?>
                        <option value="<?php echo $pk->package_id?>"><?php echo $pk->package?></option>
                        <?php } ?>
                    </select>
                </div>
                <div class="data" style="width:30%">
                ...
                <div class="data" style="width:auto">
                    <button type="button" id="change_category" name="change_category" style="font-size:.8rem"onclick="updateSubMenu()">Update <i class="fas fa-save"></i></button>
