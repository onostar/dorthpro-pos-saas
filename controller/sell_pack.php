<?php   
        ...
        $unit_price = $get_price->fetch_details_cond('items', 'item_id', $item_id);
        foreach($unit_price as $up){
            $markup = $up->markup;
            $old_price = $up->sales_price;
            $cost_price = $up->cost_price;
        }
        /* $old_price = $unit_price->pack_price;
        $discount = $old_price - $pack_price; */
        $discount = $pack_price * (($markup)/100);
        $new_price = $pack_price - $discount;
        ...
        if($total_qty > $inv_qty){
           ...
           /*  $get_cost = new selects();
            ...
            if($cost_price > $pack_price){
                echo "<script>alert('Sales price cannot be less than cost price!')</script>";
            }else{
                //update quantity and price
                $update = new Update_table();
                $update->update_multiple('sales', 'quantity', $total_qty, 'price', $pack_price, 'total_amount', $new_amount, 'cost', $total_cost, 'discount', $discount, 'sales_id', $sales);
