    ...
    // if(isset($_POST['change_prize'])){
      ...
      /*   $inser_trail = new audit_trail($item, $trans_type, $inv_qty, $quantity, $user, $store);
        ...
        if($modes > 1){
            ...
            if($payment_type !== "Credit" && $payment_type !== "Deposit"){
        ...
        // if($update_payment){
            ...
            if($sales_return){
                ...
                if($payment_type == "Credit"){
                    if($new_inv_due == 0){
                        //delete from debtors
                        $delete_debt = new deletes();
                        $delete_debt->delete_item('debtors', 'invoice', $invoice);
                        //delete from customer trail
                        $delete_debt->delete_item('customer_trail', 'invoice', $invoice);
                    }else{
                        //update  debtors
                        $update_debt = new Update_table();
                        $update_debt->update('debtors', 'amount', 'invoice', $new_inv_due, $invoice);
                        //update customer trail
                        $update_debt->update('customer_trail', 'amount', 'invoice', $new_inv_due, $invoice);
                    }
                ...
                if($payment_type == "Deposit"){
                    if($new_inv_due == 0){
                        ...
                        $new_debt = $new_inv_due - $invoice_amount;
                        //update  debtors
                        $update_debt = new Update_table();
                        $update_debt->update('debtors', 'amount', 'invoice', $new_debt, $invoice);
                        //update customer trail
                        $update_debt->update('customer_trail', 'amount', 'invoice', $new_debt, $invoice);
                    }
