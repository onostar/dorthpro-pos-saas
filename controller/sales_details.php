...
<div class="displays allResults" id="stocked_items">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="color:red;font-size:.8rem">
                    ...
                    <a style="color:#fff; background:var(--secondaryColor);border-radius:4px;padding:5px 8px;" href="javascript:void(0)" title="sell item in pack" onclick="getPack('<?php echo $detail->sales_id?>')"><i class="fas fa-box"></i></a>
                </td>
                <td>
                    <?php 
    ...
    <?php
        ...
        //total discount
        $disc = $get_total->fetch_sum_con('sales', 'discount', 'quantity', 'invoice', $invoice);
        foreach($disc as $dis){
            $total_discount = $dis->total;
        }
    ...
    <section style="float:right;margin:5px;">
        <?php 
        //display discount
            // echo "<p class='total_amount' style='background:none; color:green; text-decoration:underline; width:auto; float:right; padding:0;font-size:.9rem; display:block;'>Total Discount (10%): ₦".number_format($total_discount, 2)."</p>";
        ?>
        <!-- <label for="discount" style="color:red;">Discount</label><br> -->
        <input type="hidden" name="discount" id="discount" style="padding:5px;border-radius:5px;" value="<?php echo $total_discount?>">
    </section>
    <?php
        if(gettype($details) == "array"){
    ?>
