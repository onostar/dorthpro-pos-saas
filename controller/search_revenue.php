<?php
    ...
    $n = 1;
?>
<h2>Revenue By Invoice Report between '<?php echo date("jS M, Y", strtotime($from)) . "' and '" . date("jS M, Y", strtotime($to))?>'</h2>
    <hr>
    <div class="search">
...
?>
            <tr>
                ...
                <td>
                    <?php
                        //get customer
                        $cus = $get_revenue->fetch_details_cond('customers', 'customer_id', $detail->customer);
                        if(is_array($cus)){
                            foreach($cus as $cu){
