<?php
    ...
    class selects extends Dbh{
        ...
        public function fetch_details($table){
         ...
         //fetch basic package
        public function fetch_basic_package($menu){
            $get_user = $this->connectdb()->prepare("SELECT * FROM sub_menus WHERE package = 1 AND status = 0 AND menu = :menu");
            $get_user->bindValue("menu", $menu);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
         ...
         //fetch standard package
        public function fetch_standard_package($menu){
            $get_user = $this->connectdb()->prepare("SELECT * FROM sub_menus WHERE (package = 1 OR package = 2) AND status = 0 AND menu = :menu");
            $get_user->bindValue("menu", $menu);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
         ...
         //fetch enterprise package
        public function fetch_enterprise($menu){
            $get_user = $this->connectdb()->prepare("SELECT * FROM sub_menus WHERE status = 0 AND menu = :menu");
            $get_user->bindValue("menu", $menu);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //FECTH DETAILSLIKE 2 NEW
        public function fetch_details_like2Equalsnew($table, $column1, $column2, $value, $type) {
            try {
                if ($type === "barcode") {
                    $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $column2 = :value");
                    $get_user->bindValue(":value", $value);
                } else {
                    $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $column1 LIKE :value");
                    $get_user->bindValue(":value", "%$value%");
                }
                $get_user->execute();
        ...
        //fetch between two dates and 2 condition
        public function fetch_details_2date2Con1neg($table, $column, $value1, $value2, $condition, $condition_value, $condition2, $condition_value2, $neg_con, $neg_val){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition = :$condition AND $condition2 = :$condition2 AND $neg_con != :$neg_con AND $column BETWEEN '$value1' AND '$value2'");
            $get_user->bindValue("$condition",$condition_value);
            $get_user->bindValue("$condition2",$condition_value2);
            $get_user->bindValue("$neg_con",$neg_val);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch between two dates and 2 condition
        public function fetch_details_2date2ConOrder($table, $column, $value1, $value2, $condition, $condition_value, $condition2, $condition_value2, $order){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition = :$condition AND $condition2 = :$condition2 AND $column BETWEEN '$value1' AND '$value2' ORDER BY $order");
            $get_user->bindValue("$condition",$condition_value);
            $get_user->bindValue("$condition2",$condition_value2);
            $get_user->execute();
            if($get_user->rowCount() > 0){
        ...
        //fetch between two dates and a condition and grouped
        public function fetch_details_dateGro1conOrder($table, $condition1, $value1, $value2, $con, $con_value, $group, $order){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $con = :$con AND $condition1 BETWEEN '$value1' AND '$value2' GROUP BY $group ORDER BY $order");
            $get_user->bindValue("$con", $con_value);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //fetch with current date and a condition grouped by condition
        public function fetch_details_curdateGro1conOrder($table, $column, $condition, $value, $group, $order){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE date($column) = CURDATE() AND $condition = :$condition GROUP BY $group ORDER BY $order");
            $get_user->bindValue("$condition", $value);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        public function fetch_salesOrder($store){
            $get_user = $this->connectdb()->prepare("SELECT SUM(total_amount) AS total, customer, invoice, posted_by, post_date FROM sales WHERE sales_status = 1 AND store = :store AND date(post_date) = CURDATE() GROUP BY invoice ORDER BY post_date");
            $get_user->bindValue("store", $store);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //fetch sales order with current date
        public function fetch_salesOrderRep($store, $rep){
            $get_user = $this->connectdb()->prepare("SELECT SUM(total_amount) AS total, invoice, posted_by, customer, post_date FROM sales WHERE sales_status = 1 AND posted_by = :posted_by AND store = :store AND date(post_date) = CURDATE() GROUP BY invoice ORDER BY post_date");
            $get_user->bindValue("posted_by", $rep);
            $get_user->bindValue("store", $store);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        public function fetch_cashier_report($store){
            $get_daily = $this->connectdb()->prepare("SELECT * FROM payments WHERE store = :store AND date(post_date) = CURDATE() GROUP BY posted_by ORDER BY posted_by");
        ...
        public function fetch_cashier_reportDate($store, $start, $end){
            $get_daily = $this->connectdb()->prepare("SELECT * FROM payments WHERE store = :store AND date(post_date) BETWEEN '$start' AND '$end' GROUP BY posted_by ORDER BY posted_by");
            $get_daily->bindValue('store', $store);
            // $get_daily->bindValue('start_dates', $start);
            // $get_daily->bindValue('end_dates', $end);
            $get_daily->execute();
        ...
        //fetch details with date and 2 conditions
        public function fetch_details_date2Cond1neg($table, $column, $condition1, $value1, $condition2, $value2, $neg_con, $neg_val){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition1 = :$condition1 AND $condition2 = :$condition2 AND $neg_con != :$neg_con AND $column = CURDATE()");
            $get_user->bindValue("$condition1", $value1);
            $get_user->bindValue("$condition2", $value2);
            $get_user->bindValue("$neg_con", $neg_val);
            $get_user->execute();
        ...
        //fetch details with date and 2 conditions order
        public function fetch_details_date2CondOrder($table, $column, $condition1, $value1, $condition2, $value2, $order){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition1 = :$condition1 AND $condition2 = :$condition2 AND $column = CURDATE() ORDER BY $order");
            $get_user->bindValue("$condition1", $value1);
            $get_user->bindValue("$condition2", $value2);
            // $get_user->bindValue("$condition2", $value2);
            $get_user->execute();
        ...
        public function fetch_items_sales($store, $item, $type){
            if($type == "barcode"){
                $get_user = $this->connectdb()->prepare("SELECT i.item_id, i.item_name, i.sales_price, IFNULL(SUM(inv.quantity), 0) AS quantity FROM items i LEFT JOIN inventory inv ON inv.item = i.item_id AND inv.store = :store WHERE i.barcode = :item GROUP BY i.item_id LIMIT 20");
                $get_user->bindValue("store", $store);
                $get_user->bindValue("item", $item);
            }else{
                $get_user = $this->connectdb()->prepare("SELECT  i.item_id, i.item_name, i.sales_price, i.wholesale, IFNULL(SUM(inv.quantity), 0) AS quantity FROM items i LEFT JOIN inventory inv ON inv.item = i.item_id AND inv.store = :store WHERE i.item_name LIKE :item GROUP BY i.item_id ORDER BY i.item_name ASC LIMIT 30");
                $get_user->bindValue("store", $store);
                $get_user->bindValue("item", "%$item%");
            }
            $get_user->execute();
        ...
        //fetch stock balance
        public function fetch_stock_balance($store){
            $get_user = $this->connectdb()->prepare(" SELECT i.item_id, i.item_name, i.department, d.department AS category_name, inv.cost_price, SUM(inv.quantity) AS total_qty FROM inventory inv JOIN items i ON inv.item = i.item_id JOIN departments d ON i.department = d.department_id WHERE inv.store = :store GROUP BY i.item_id HAVING SUM(inv.quantity) > 0 ORDER BY i.item_name ASC");
            $get_user->bindValue("store", $store);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //fetch weekly sales
        public function fetch_week_sales($store, $start, $end){
            $get_monthly = $this->connectdb()->prepare("SELECT SUM(total_amount) AS revenue, post_date, SUM(cost) AS total_cost FROM sales WHERE store = :store AND sales_status = 2 AND date(post_date) BETWEEN '$start' AND '$end'");
            $get_monthly->bindValue('store', $store);
            $get_monthly->execute();
            if($get_monthly->rowCount() > 0){
                $rows = $get_monthly->fetchAll();
        ...
        //fetch weekly expense
        public function fetch_week_expense($store, $start, $end){
            $get_monthly = $this->connectdb()->prepare("SELECT SUM(amount) AS expense FROM expenses WHERE store = :store AND date(post_date) BETWEEN '$start' AND '$end'");
            $get_monthly->bindValue('store', $store);
            $get_monthly->execute();
            if($get_monthly->rowCount() > 0){
                $rows = $get_monthly->fetchAll();
        ...
        //fetch weekly highest selling items
        public function fetch_weekly_highest($store, $start, $end, $order){
            $get_monthly = $this->connectdb()->prepare("SELECT item, SUM(total_amount) AS revenue, SUM(quantity) AS total_quantity, SUM(cost) AS total_cost FROM sales WHERE store = :store AND sales_status = 2 AND date(post_date) BETWEEN '$start' AND '$end' GROUP BY item ORDER BY $order DESC LIMIT 1");
            $get_monthly->bindValue('store', $store);
            $get_monthly->execute();
            if($get_monthly->rowCount() > 0){
                $rows = $get_monthly->fetchAll();
        ...
        //fetch weekly best cashier
        public function fetch_weekly_cashier($store, $start, $end){
            $get_monthly = $this->connectdb()->prepare("SELECT COUNT(distinct invoice) AS customers, SUM(total_amount) AS revenue, posted_by FROM sales WHERE store = :store AND sales_status = 2 AND date(post_date) BETWEEN '$start' AND '$end' GROUP BY posted_by ORDER BY COUNT(distinct invoice) DESC LIMIT 1");
            $get_monthly->bindValue('store', $store);
            $get_monthly->execute();
            if($get_monthly->rowCount() > 0){
                $rows = $get_monthly->fetchAll();
        ...
        //fetch low stock item
        public function fetch_low_stock($store){
            $get_monthly = $this->connectdb()->prepare("SELECT item, SUM(quantity) AS total_quantity FROM inventory WHERE store = :store GROUP BY item HAVING SUM(quantity) BETWEEN 1 AND 5 ORDER BY total_quantity ASC LIMIT 1");
            $get_monthly->bindValue('store', $store);
            $get_monthly->execute();
            if($get_monthly->rowCount() > 0){
                $rows = $get_monthly->fetchAll();
